...
import { unicodeArrows, fixObsidianDashes, obsidianWikilinks, paragraphLinks, obsidianReplaceEmptyTableRow } from './src/util/markdown-plugins';
import { wikilinksToHypertextLinks, wikilinksToMdLinks } from './src/util/wikilinks';
import { getSlug } from './src/util/getSlug';
const files = await readdir('./src/content/brain');
const filesProc = files.filter(file => !file.startsWith('.')).map(file => getSlug(file.split('.')[0]));
...
export default defineConfig({
  ...
  integrations: [tailwind(), prefetch(), icon(), markdoc()],
  markdown: {
    remarkPlugins: [
    // mmmm, curry
    () => (ast, file) => {
      ...
      if (isObsidian(file)) {
        ...
        visit(ast, 'tableRow', obsidianReplaceEmptyTableRow(emptyRow => Object.assign(emptyRow, {
          type: 'html',
          value: `<tr style="box-shadow: inset 0 1px 0 0 #3f3f46; height: 1px;"></tr>`
        })));
      }
    }],
    rehypePlugins: [() => (ast, file) => {
